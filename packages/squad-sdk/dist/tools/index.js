 ...
 */
function sanitizeErrorForLlm(error, squadRoot) {
...
export class ToolRegistry {
    ...
    squadRoot;
    sessionPoolGetter;
    constructor(squadRoot = '.squad', sessionPoolGetter) {
    ...
    registerSquadTools() {
        ...
        const squadStatus = defineTool({
            ...
            handler: async (args) => {
                ...
                const allSessions = Array.from(pool.sessions?.values() || []);
                let filteredSessions = allSessions;
                // Apply agent name filter
                if (args.agentName) {
                    filteredSessions = filteredSessions.filter((s) => s.agentName === args.agentName);
                }
                // Apply status filter
                if (args.status) {
                ...
                const poolInfo = {
                    poolSize: pool.size,
                    capacity: pool.config?.maxConcurrent || 0,
