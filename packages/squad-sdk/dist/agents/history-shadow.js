...
`;
        ...
        throw new ConfigurationError(`Failed to create history shadow for agent '${agentName}': ${error instanceof Error ? error.message : String(error)}`, {
            agentName,
...
export async function appendToHistory(teamRoot, agentName, section, content) {
    try {
        ...
        throw new ConfigurationError(`Failed to append to history for agent '${agentName}': ${error instanceof Error ? error.message : String(error)}`, {
            agentName,
            operation: 'appendToHistory',
            timestamp: new Date(),
...
export async function readHistory(teamRoot, agentName) {
    try {
        ...
        throw new ConfigurationError(`Failed to read history for agent '${agentName}': ${error instanceof Error ? error.message : String(error)}`, {
            agentName,
...
//# sourceMappingURL=history-shadow.js.map