 ...
 */
export function compileCharter(options) {
 ...
 * @throws {ConfigurationError} If charter is missing or malformed
 */
export function compileCharterFull(options) {
    ...
    try {
        ...
        if (error instanceof ConfigurationError)
            throw error;
        throw new ConfigurationError(`Failed to compile charter for agent '${agentName}' at ${charterPath}: ${error instanceof Error ? error.message : String(error)}`, {
            agentName,
 ...
 */
export function parseCharterMarkdown(content) {
    ...
    if (identityMatch) {
        ...
        if (nameMatch)
            result.identity.name = nameMatch[1].trim();
        const roleMatch = identityContent.match(/\*\*Role:\*\*\s*(.+)/i);
        if (roleMatch)
            result.identity.role = roleMatch[1].trim();
        ...
        if (styleMatch)
            result.identity.style = styleMatch[1].trim();
    }
