...
export async function initSquad(options) {
    ...
    // -------------------------------------------------------------------------
    // Materialize system sub-agent charters from templates
    // -------------------------------------------------------------------------
    // Some agents are installed as fully-authored charter templates rather than
    // generated by `generateCharter()`. These are typically system agents whose
    // charters are tightly coupled to coordinator behavior (e.g., the Spec
    // family — spec-constitution, spec-prd, spec-feature, spec-index — required
    // by Phase Routing in squad.agent.md.template).
    //
    // For each `templates/agents/{name}/charter.md` shipped in the SDK's
    // templates directory, copy it to `.squad/agents/{name}/charter.md` if the
    // live charter doesn't already exist. writeIfNotExists semantics — never
    // overwrite user-customized charters at init time.
    if (templatesDir) {
        const templatesAgentsDir = join(templatesDir, 'agents');
        if (existsSync(templatesAgentsDir)) {
            const templateAgentNames = readdirSync(templatesAgentsDir, { withFileTypes: true })
                .filter(ent => ent.isDirectory())
                .map(ent => ent.name);
            for (const agentName of templateAgentNames) {
                const templateCharterPath = join(templatesAgentsDir, agentName, 'charter.md');
                if (!existsSync(templateCharterPath))
                    continue;
                const liveAgentDir = join(agentsDir, agentName);
                const liveCharterPath = join(liveAgentDir, 'charter.md');
                // Skip if this agent was already created in the loop above
                // (writeIfNotExists semantics — don't overwrite generated charters or user customizations)
                if (existsSync(liveCharterPath))
                    continue;
                await mkdir(liveAgentDir, { recursive: true });
                const charterContent = readFileSync(templateCharterPath, 'utf-8');
                await writeIfNotExists(liveCharterPath, charterContent);
                agentDirs.push(liveAgentDir);
                // Initialize history.md for the system agent (writeIfNotExists)
                const historyPath = join(liveAgentDir, 'history.md');
                const historyContent = generateInitialHistory({ name: agentName, role: agentName, displayName: agentName }, projectName, projectDescription, userName);
                await writeIfNotExists(historyPath, historyContent);
            }
        }
    }
