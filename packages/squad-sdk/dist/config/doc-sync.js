 ...
 */
export function syncDocToConfig(doc, config) {
    // --- Team-level fields ---
    if (doc.name) {
    ...
    if (doc.routingHints.length > 0) {
        ...
        const existingPatterns = new Set(config.routing.rules.map((r) => r.pattern));
    ...
    if (config.agents.length > 0) {
        ...
        if (doc.capabilities.length > 0)
            agent.charter = doc.capabilities.join('\n');
        if (doc.modelPreferences.length > 0)
            agent.model = doc.modelPreferences[0];
 ...
 */
export function syncConfigToDoc(config, template) {
 ...
 */
export function detectDrift(doc, config) {
    const entries = [];
    // --- Team name ---
...
function buildCapabilitiesBlock(config) {
    if (config.agents.length === 0)
...
function buildRoutingBlock(config) {
    if (config.routing.rules.length === 0)
        return '_No routing rules defined._';
    const lines = [];
