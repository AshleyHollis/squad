 ...
 */
export function parseTeamMarkdown(content) {
 ...
 */
export function parseRoutingRulesMarkdown(content) {
    content = normalizeEol(content);
    const rules = [];
    const warnings = [];
    if (!content || !content.trim()) {
 ...
 */
export function parseDecisionsMarkdown(content) {
    content = normalizeEol(content);
    const decisions = [];
    const warnings = [];
    if (!content || !content.trim()) {
    ...
    const flush = () => {
        if (currentTitle) {
            ...
            const decision = { title: currentTitle, body, configRelevant };
            if (currentDate)
                decision.date = currentDate;
            if (author)
                decision.author = author;
            if (currentHeadingLevel !== undefined)
                decision.headingLevel = currentHeadingLevel;
 ...
 */
export function generateConfigFromParsed(parsed, base) {
    const baseConfig = base ?? {};
    // Build routing rules
    const routingRules = parsed.routingRules.map((r) => ({
        workType: r.workType,
 ...
 */
export function migrateMarkdownToConfig(options) {
    const warnings = [];
    ...
    const parsed = { agents, routingRules, decisions, warnings };
...
function finalizeAgent(partial) {
    return {
...
//# sourceMappingURL=markdown-migration.js.map