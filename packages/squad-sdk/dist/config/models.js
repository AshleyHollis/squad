...
export class ModelRegistry {
    ...
    constructor(catalog = MODEL_CATALOG) {
        ...
        for (const tier of ['premium', 'standard', 'fast']) {
            this.tierIndex.set(tier, catalog.filter(m => m.tier === tier));
        }
     ...
     */
    getFallbackChain(tier, preferSameProvider = true, currentModel) {
     ...
     */
    getNextFallback(currentModel, tier, attemptedModels = new Set()) {
     ...
     */
    getRecommendedModels(useCase, tier) {
        ...
        const scored = candidates
            .map(model => ({
            model,
            score: model.useCases?.some(uc => uc.toLowerCase().includes(useCaseLower) ||
                useCaseLower.includes(uc.toLowerCase())) ? 10 : 0
        }))
 ...
 */
export function getFallbackChain(tier) {
...
export function estimateCost(model, inputTokens, outputTokens) {
    const info = defaultRegistry.getModelInfo(model);
    if (!info?.pricing)
        return 0;
 ...
 */
export const ECONOMY_MODEL_MAP = {
 ...
 */
export function applyEconomyMode(model) {
 ...
 */
export function readEconomyMode(squadDir) {
 ...
 */
export function writeEconomyMode(squadDir, enabled) {
    const configPath = join(squadDir, 'config.json');
    let config = {};
    if (existsSync(configPath)) {
        try {
 ...
 */
export function readModelPreference(squadDir) {
    const configPath = join(squadDir, 'config.json');
    if (!existsSync(configPath)) {
        return null;
    }
 ...
 */
export function readAgentModelOverrides(squadDir) {
    const configPath = join(squadDir, 'config.json');
    if (!existsSync(configPath)) {
        return {};
    }
 ...
 */
export function writeModelPreference(squadDir, model) {
    const configPath = join(squadDir, 'config.json');
    let config = {};
    if (existsSync(configPath)) {
        try {
 ...
 */
export function writeAgentModelOverrides(squadDir, overrides) {
    const configPath = join(squadDir, 'config.json');
    let config = {};
    if (existsSync(configPath)) {
        try {
...
export function resolveModel(options) {
    ...
    const isEconomy = options.economyMode !== undefined
        ? options.economyMode
