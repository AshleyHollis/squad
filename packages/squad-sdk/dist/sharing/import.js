 ...
 */
export function deserializeBundle(content) {
...
export function validateBundle(bundle) {
    ...
    if (!bundle.agents || !Array.isArray(bundle.agents)) {
        ...
        for (let i = 0; i < bundle.agents.length; i++) {
            ...
            if (!agent.name)
                errors.push({ field: `agents[${i}].name`, message: 'Agent missing name' });
            if (!agent.content)
                errors.push({ field: `agents[${i}].content`, message: 'Agent missing content' });
        }
    }
 ...
 */
export function importSquadConfig(bundlePath, targetDir, options) {
    const opts = {
    ...
    try {
        ...
        return { success: false, changes: [], warnings: [`Failed to parse bundle: ${err.message}`] };
    }
    // Validate
...
function formatRoutingRules(rules) {
