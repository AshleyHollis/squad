 ...
 * @module runtime/otel
 */
import { trace, metrics, diag, DiagConsoleLogger, DiagLogLevel } from './otel-api.js';
...
function buildResource(config, Resource) {
    if (!Resource)
        return undefined;
...
function ensureSDK(config) {
    ...
    const req = createRequire(import.meta.url);
    let NodeSDK, Resource, PeriodicExportingMetricReader; // eslint-disable-line @typescript-eslint/no-explicit-any
    let OTLPTraceExporter, OTLPMetricExporter; // eslint-disable-line @typescript-eslint/no-explicit-any
    try {
