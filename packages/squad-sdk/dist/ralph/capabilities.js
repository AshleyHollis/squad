...
export function getDeploymentMode() {
    ...
    if (raw === 'squad-per-pod')
        return 'squad-per-pod';
 ...
 */
export function generatePodCapabilitiesPath(teamRoot, podId) {
 ...
 */
export async function loadCapabilities(teamRoot) {
    const candidates = [];
 ...
 *   // → ['gpu', 'browser']
 */
export function extractNeeds(labels) {
    return labels
 ...
 */
export function canHandleIssue(issueLabels, capabilities) {
 ...
 */
export function filterByCapabilities(issues, capabilities) {
    const handled = [];
    const skipped = [];
