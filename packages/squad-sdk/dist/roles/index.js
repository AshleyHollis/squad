 ...
 */
export function useRole(roleId, options) {
    const role = getRoleById(roleId);
    if (!role) {
        const available = BASE_ROLES.map(r => r.id).join(', ');
        throw new Error(`Unknown base role '${roleId}'. Available roles: ${available}`);
    }
 ...
 */
function buildCharterFromRole(role, overrides) {
    const lines = [];
 ...
 */
export function generateCharterFromRole(roleId, agentName) {
    const role = getRoleById(roleId);
    if (!role)
        return null;
    ...
    return `# ${agentName} — ${role.title}\n\n> ${role.vibe}\n\n${charter.replace('{my-name}', nameLower)}`;
}
