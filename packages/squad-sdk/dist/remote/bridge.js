...
import { RC_PROTOCOL_VERSION, serializeEvent, parseCommand, } from './protocol.js';
export class RemoteBridge {
    ...
    messageIdCounter = 0;
    staticHandler;
    acpEventLog = []; // Records all ACP events for replay
    wsRateLimit = new Map();
    ipConnections = new Map();
    httpRateLimits = new Map();
    sessionToken = randomUUID();
    tickets = new Map(); // F-02: one-time WS tickets
    SESSION_TTL = 4 * 60 * 60 * 1000; // F-18: 4-hour session TTL
    sessionCreatedAt = Date.now();
    auditLogDir = path.join(os.homedir(), '.cli-tunnel', 'audit');
    auditLogPath = path.join(this.auditLogDir, `squad-audit-${Date.now()}.jsonl`);
    auditLog = (() => { fs.mkdirSync(this.auditLogDir, { recursive: true, mode: 0o700 }); return fs.createWriteStream(this.auditLogPath, { flags: 'a' }); })();
    constructor(config) {
        this.config = config;
    ...
    async start() {
        ...
        this.wss = new WebSocketServer({
            ...
            verifyClient: (info) => {
                ...
                if (url.searchParams.get('token') !== this.sessionToken)
                    return false;
                return true;
            },
        });
    ...
    addMessage(role, content, agentName) {
        const msg = {
    ...
    sendDelta(sessionId, agentName, content) {
    ...
    handleDeleteSession(tunnelId, res) {
        try {
            ...
            res.writeHead(200, { 'Content-Type': 'application/json', 'X-Content-Type-Options': 'nosniff' });
            res.end(JSON.stringify({ deleted: false, error: err.message }));
        }
    }
    ...
    handleConnection(ws, req) {
        ...
        if (this.config.enableReplay && this.passthroughWrite && this.acpEventLog.length > 0) {
            for (const event of this.acpEventLog) {
                this.send(ws, { type: '_replay', data: this.redactSecrets(event) });
            }
            this.send(ws, { type: '_replay_done' });
        }
        else {
