 ...
 */
export function packageForMarketplace(projectDir, manifest) {
    const warnings = [];
    const files = [];
 ...
 */
export function validatePackageContents(packagePath) {
    const errors = [];
    const missingFiles = [];
    ...
    const hasIcon = iconCandidates.some((ic) => fs.existsSync(path.join(packagePath, ic)));
    if (!hasIcon) {
