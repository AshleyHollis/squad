    ...
    for (const trigger of config.triggers) {
        if (trigger.event === 'schedule' && trigger.cron) {
            ...
            if (trigger.branches?.length) {
                lines.push('    branches:');
                for (const b of trigger.branches)
                    lines.push(`      - ${b}`);
            }
            if (trigger.tags?.length) {
                lines.push('    tags:');
                for (const t of trigger.tags)
                    lines.push(`      - ${t}`);
            }
        }
 ...
 */
export function generateReleaseWorkflow(config) {
    const releaseConfig = {
        ...config,
...
export function validatePipelineConfig(config) {
    ...
    if (!config.steps || config.steps.length === 0) {
        ...
        for (let i = 0; i < config.steps.length; i++) {
            ...
            if (!step.name)
                errors.push(`Step ${i} missing name`);
            if (!step.command)
                errors.push(`Step ${i} missing command`);
        }
    }
