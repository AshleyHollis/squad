  ...
  if (templatesDir) {
    ...
    if (existsSync(templatesAgentsDir)) {
      ...
      for (const agentName of templateAgentNames) {
        ...
        if (!existsSync(templateCharterPath)) continue;
        ...
        if (existsSync(liveCharterPath)) continue;
        ...
        const historyContent = generateInitialHistory(
          { name: agentName, role: agentName, displayName: agentName },
          projectName,
          projectDescription,
          userName
        );
