...
process.emit = function (evt, ...args) {
    if (evt === 'warning' && args[0]?.name === 'ExperimentalWarning') {
        return false;
    }
    return _origEmit.apply(this, [evt, ...args]);
};
...
async function main() {
    ...
    if (cmd === 'migrate') {
        ...
        await runMigrate(process.cwd(), { to, from: from, dryRun });
        return;
    }
