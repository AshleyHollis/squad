...
export class ShellLifecycle {
    ...
    async initialize() {
        ...
        if (!fs.existsSync(squadDir) || !fs.statSync(squadDir).isDirectory()) {
            ...
            const err = new Error(`No team found. Run \`squad init\` to create one.`);
        ...
        if (!fs.existsSync(teamPath)) {
            this.state.status = 'error';
            const err = new Error(`No team manifest found. The .squad/ directory exists but has no team.md. Run \`squad init\` to fix.`);
...
export function getRoleEmoji(role) {
    ...
    if (normalized.includes('backend') || normalized.includes('api') || normalized.includes('server'))
        return '🔧';
    if (normalized.includes('test') || normalized.includes('qa') || normalized.includes('quality'))
        return '🧪';
    if (normalized.includes('game') || normalized.includes('logic'))
        return '🎮';
    if (normalized.includes('devops') || normalized.includes('infra') || normalized.includes('platform'))
        return '⚙️';
    if (normalized.includes('security') || normalized.includes('auth'))
        return '🔒';
    if (normalized.includes('doc') || normalized.includes('writer') || normalized.includes('devrel'))
        return '📝';
    if (normalized.includes('data') || normalized.includes('database') || normalized.includes('analytics'))
        return '📊';
    if (normalized.includes('design') || normalized.includes('visual') || normalized.includes('graphic'))
        return '🎨';
...
export function loadWelcomeData(teamRoot) {
    try {
