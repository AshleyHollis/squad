...
export async function runShell() {
    ...
    const _bridge = new StreamBridge(registry, {
        ...
        onComplete: (message) => {
            if (message.agentName)
                streamBuffers.delete(message.agentName);
    ...
    async function awaitStreamedResponse(session, prompt) {
        if (session.sendAndWait) {
            ...
            const done = new Promise((resolve) => {
                const onEnd = () => {
                    try {
                        session.off('turn_end', onEnd);
                    }
                    catch { /* ignore */ }
                    try {
                        session.off('idle', onEnd);
                    }
                    catch { /* ignore */ }
                    resolve();
                };
    ...
    async function dispatchToCoordinator(message) {
        ...
        if (decision.type === 'route' && decision.routes?.length) {
            ...
            await Promise.allSettled(decision.routes.map(r => dispatchToAgent(r.agent, r.task)));
        }
        else {
    ...
    try {
        ...
        if (existsSync(configPath)) {
            ...
            if (parsed && typeof parsed === 'object' && parsed.consult === true) {
