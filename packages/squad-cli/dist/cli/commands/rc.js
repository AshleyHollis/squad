...
export async function runRC(cwd, options) {
    ...
    if (squadDir) {
        try {
            ...
            for (const line of lines) {
                ...
                if (cols.length < 2 || cols[0] === 'Name' || /^[-:]+$/.test(cols[0]))
                    continue;
    ...
    const allAgents = copilotReady
        ? [{ name: 'Copilot', role: 'AI Assistant', status: 'idle' }, ...agents.map(a => ({ name: a.name, role: a.role, status: 'idle' }))]
        : agents.map(a => ({ name: a.name, role: a.role, status: 'idle' }));
    ...
    try {
        ...
        console.log(`  ${YELLOW}⚠${RESET} Copilot not available: ${err.message}\n`);
    }
    // Tunnel setup
    if (options.tunnel) {
        if (!isDevtunnelAvailable()) {
            ...
            try {
                ...
                try {
                    ...
                    qrcode.default.generate(tunnelWithToken, { small: true }, (code) => {
                        console.log(code);
                    });
                }
                catch {
