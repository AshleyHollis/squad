...
import { isConsultMode, detectLicense, loadStagedLearnings, logConsultation, mergeToPersonalSquad, getPersonalSquadRoot, } from '@bradygaster/squad-sdk';
 ...
 */
async function promptSelection(items, preselected) {
    const rl = createInterface({
        input: process.stdin,
        output: process.stdout,
    });
 ...
 */
export async function runExtract(cwd, args) {
    ...
    if (!existsSync(configPath)) {
        fatal('No .squad/config.json found.\n' +
    ...
    if (config.extractionDisabled && !force) {
        fatal('Extraction is disabled for this consult session.\n' +
    ...
    if (resolve(sourceSquad) !== resolve(expectedPersonalSquad)) {
        fatal(`sourceSquad path mismatch.\n` +
