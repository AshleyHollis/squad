 ...
 */
function checkRateLimitStatus(squadDir) {
    const statusPath = path.join(squadDir, 'rate-limit-status.json');
    if (!fileExists(statusPath))
        return undefined;
    const data = tryReadJson(statusPath);
    if (!data) {
        return {
 ...
 */
function checkVscodeJsonrpcExports(cwd) {
    const possiblePaths = [
 ...
 */
function checkCopilotSdkSessionPatch(cwd) {
    const possiblePaths = [
...
function checkSquadAgentMd(cwd) {
