 ...
 */
export async function runSubSquads(cwd, args) {
    const sub = args[0];
    if (!sub || sub === 'list') {
 ...
 */
function showSubSquadStatus(cwd) {
    const config = loadSubSquadsConfig(cwd);
    const active = resolveSubSquad(cwd);
    if (!config || config.workstreams.length === 0) {
        console.log(`\n${DIM}No SubSquads configured.${RESET}\n`);
    ...
    for (const subsquad of config.workstreams) {
        ...
        try {
            const result = spawnSync('gh', ['pr', 'list', '--label', subsquad.labelFilter, '--json', 'number,title,state', '--limit', '5'], { cwd, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
        ...
        try {
            const result = spawnSync('git', ['branch', '--list', `*${subsquad.name}*`], { cwd, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
 ...
 */
function activateSubSquad(cwd, name) {
    const config = loadSubSquadsConfig(cwd);
