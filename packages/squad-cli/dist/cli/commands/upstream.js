...
function isValidUpstreamName(name) {
    return /^[a-zA-Z0-9._-]+$/.test(name);
}
function readUpstreams(upstreamFile) {
...
function writeUpstreams(upstreamFile, data) {
...
function ensureGitignoreEntry(repoDir, entry) {
...
export async function upstreamCommand(args) {
    const action = args[0];
    ...
    if (action === 'add') {
        ...
        if (type === 'git') {
            ...
            try {
                ...
                warn(`Clone failed — run "squad upstream sync" to retry: ${err.message}`);
            }
        }
        ...
        for (const upstream of toSync) {
            if (upstream.type === 'local' || upstream.type === 'export') {
                ...
                try {
                    ...
                    warn(`${upstream.name}: git sync failed: ${err.message}`);
                }
            }
        }
        writeUpstreams(upstreamFile, data);
