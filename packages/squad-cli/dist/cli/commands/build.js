 ...
 *   3. squad.config.js
 */
async function loadSquadConfig(cwd) {
    const candidates = [
    ...
    for (const candidate of candidates) {
        if (fs.existsSync(candidate)) {
            try {
                ...
                const config = mod.default ?? mod.config ?? mod.squadConfig;
                if (!config) {
    ...
    fatal('No squad config found. Create squad.config.ts or squad/index.ts\n' +
        '       Run "squad init" to bootstrap a new project.');
}
// ---------------------------------------------------------------------------
// Markdown generators
// ---------------------------------------------------------------------------
function generateTeamMd(config) {
...
function generateRoutingMd(config) {
    if (!config.routing)
        return '';
    const lines = [GENERATED_HEADER];
    const teamName = config.team.name;
 ...
 */
function normalizeModelPreference(model) {
    if (!model)
        return undefined;
 ...
 */
function renderModelSection(agentModel, defaultModel) {
...
function generateCharterMd(agent, defaultModel) {
    const lines = [GENERATED_HEADER];
...
function generateCeremoniesMd(ceremonies) {
    const lines = [GENERATED_HEADER];
 ...
 */
function generateCeremoniesDispatchTable(ceremonies) {
    const lines = [GENERATED_HEADER];
    lines.push('# Ceremonies\n');
...
function generateCeremonySkillFile(c) {
    const lines = [GENERATED_HEADER];
 ...
 */
export async function runBuild(cwd, options = {}) {
    // --watch stub
    if (options.watch) {
