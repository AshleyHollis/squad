...
function humanTokens(bytes) {
    const tokens = Math.round((bytes / 1024) * TOKENS_PER_KB);
    if (tokens < 1000)
        return `${tokens}`;
    return `${(tokens / 1000).toFixed(0)}K`;
}
function daysAgoFromLine(line) {
...
// ─── Metrics collection ─────────────────────────────────────────────────
function collectMetrics(squadDir) {
...
// ─── History compression ────────────────────────────────────────────────
function compressHistory(filePath, keepEntries, dryRun) {
    ...
    if (sections.length <= keepEntries)
        return null;
    ...
    const firstHeading = Math.min(coreStart >= 0 ? coreStart : Infinity, sections.length > 0 ? sections[0].start : Infinity);
    ...
    if (coreStart >= 0) {
        ...
        if (!newLines[newLines.length - 1]?.match(/^\s*$/))
            newLines.push('');
    }
    // Kept sections
...
// ─── Inbox cleanup ──────────────────────────────────────────────────────
function cleanInbox(squadDir, dryRun) {
    const inboxDir = path.join(squadDir, 'decisions', 'inbox');
    const decisionsFile = path.join(squadDir, 'decisions.md');
    if (!fs.existsSync(inboxDir))
        return [];
 ...
 */
function archiveDecisions(squadDir, dryRun) {
    const decisionsFile = path.join(squadDir, 'decisions.md');
    if (!fs.existsSync(decisionsFile))
        return null;
    const size = fileSize(decisionsFile);
    if (size <= DECISION_THRESHOLD)
        return null;
    ...
    if (old.length === 0) {
        ...
        if (dated.length === 0)
            return null; // only undated entries, nothing to archive
        ...
        if (old.length === 0)
            return null; // everything fits, no archival needed
...
// ─── Journal safety ─────────────────────────────────────────────────────
function checkJournal(squadDir) {
    return fs.existsSync(path.join(squadDir, JOURNAL_FILE));
}
function writeJournal(squadDir) {
    fs.writeFileSync(path.join(squadDir, JOURNAL_FILE), `nap started at ${new Date().toISOString()}\n`, 'utf8');
}
function removeJournal(squadDir) {
    const jp = path.join(squadDir, JOURNAL_FILE);
    if (fs.existsSync(jp))
        fs.unlinkSync(jp);
}
// ─── Main entry ─────────────────────────────────────────────────────────
export async function runNap(options) {
 ...
 */
export function runNapSync(options) {
    const { squadDir, deep = false, dryRun = false } = options;
    const actions = [];
    if (!fs.existsSync(squadDir)) {
        return { before: emptyMetrics(), after: emptyMetrics(), actions };
...
// ─── Report formatting ──────────────────────────────────────────────────
export function formatNapReport(result, noColor) {
...
function actionTag(type, noColor) {
    if (noColor)
...
function estimateAfterMetrics(before, actions) {
    let totalSaved = 0;
    let historySaved = 0;
    let logSaved = 0;
    let decisionSaved = 0;
    ...
    for (const a of actions) {
        ...
        if (a.type === 'compress')
            historySaved += a.bytesSaved;
        if (a.type === 'prune')
            logSaved += a.bytesSaved;
        if (a.type === 'archive')
            decisionSaved += a.bytesSaved;
        if (a.type === 'merge')
            inboxMerged++;
    }
    return {
