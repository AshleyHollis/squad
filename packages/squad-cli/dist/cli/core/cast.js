 ...
 */
export function parseCastResponse(response) {
    ...
    if (fallback && fallback.members.length > 0)
        return fallback;
    // Last resort: try emoji-prefixed lines (🏗️ Name — Role  Scope)
    const emojiResult = tryParseEmojiLines(response);
    if (emojiResult && emojiResult.members.length > 0)
        return emojiResult;
    return null;
}
function tryParseInitTeam(text) {
...
function tryParsePipeLines(text) {
...
function tryParseEmojiLines(text) {
    ...
    for (const line of lines) {
        ...
        const universeMatch = trimmed.match(/Universe:?\s*(.+)/i);
        if (universeMatch && !trimmed.includes('|'))
            universe = universeMatch[1].trim();
        const projectMatch = trimmed.match(/Project:?\s*(.+)/i);
        if (projectMatch && !trimmed.includes('|'))
            projectDescription = projectMatch[1].trim();
    }
    if (members.length === 0)
        return null;
 ...
 */
export function augmentWithCastingEngine(proposal) {
    ...
    try {
        ...
        const augmented = proposal.members.map((member, idx) => {
            ...
            if (!engineMember)
                return member; // fallback: preserve LLM data
            return {
                ...member,
...
function ownershipFromRole(role, scope) {
...
function generateCharter(member) {
...
function generateHistory(member, projectDescription) {
 ...
 */
export async function createTeam(teamRoot, proposal) {
...
export function formatCastSummary(proposal) {
    const lines = [];
